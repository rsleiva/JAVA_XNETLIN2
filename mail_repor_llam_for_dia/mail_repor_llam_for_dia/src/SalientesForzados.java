...
final class SalientesForzados {
    ...
    public void Oracle_connect() throws Exception{
        ...
        try{
            //"jdbc:oracle:thin:@ltronxgisbdpr03.pro.edenor:1528:GISPR03", "SVC_ORA_GIS", "jv506uzy"
            con_1= DriverManager.getConnection( 
    ...
    public void raster() throws Exception{
        String  plsql;
        plsql = properties.getProperty("plsql");                                //"{?=call NEXUS_GIS.LLAM_PROCESOS.LLAM_REPORT_FORZADOS_MT(?)}"; 
        try{
            Oracle_connect();					
...
//------------------------------------------------------------------------------------------------------
    private void generar(List<HashMap<String, String>> tabla, HashMap<String, String> ultima) throws Exception{
        ...
        try{		    
            ...
            hst_Mail.put("PARA"      , properties.getProperty("mail_to"));      //"Okovalow@edenor.com,CIGLESIAS@edenor.com,MRRODRIGUEZ@edenor.com"
            hst_Mail.put("CC"        , properties.getProperty("mail_cc"));      //ITSM_Desarrollos_propios@edenor.com,LD_NEXUS_PRODUCCION@edenor.com
            hst_Mail.put("ASUNTO"    , properties.getProperty("mail_subject"));  //"Información Resumen Llamadas Salientes de Documentos Forzados "
            //
